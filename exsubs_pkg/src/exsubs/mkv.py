...
def get_mkv_subtitle_tracks(video_file: Path) -> List[Dict[str, Any]]:
...
def select_track_interactively(tracks: List[dict]) -> Optional[dict]:
...
def select_best_track_for_translation(tracks: List[dict], preferred_languages: List[str] = None) -> Optional[dict]:
...
def select_best_track_with_fallback(tracks: List[dict]) -> Optional[dict]:
...
def extract_track_with_progress(video_file: Path, subtitle_file: Path, track_id: int, show_progress: bool = True) -> bool:
