...
class BatchJobConfig:
    ...
    def __init__(self, options: Options):
        ...
        self.destination_path = (
            self.options.get_str("destination_path") or "./translated"
        )
...
class BatchProcessor:
    ...
    def run(self) -> BatchStatistics:
        ...
        if not source_root.exists() or not source_root.is_dir():
            raise SubtitleError(
                f"Source path '{source_root}' does not exist or is not a directory"
            )

        destination_root = (
            pathlib.Path(self.config.destination_path).expanduser().resolve()
        )
        ...
        self.logger.info(
            ...
            self.translation_provider.selected_model or "default",
        )
        ...
        supported_extensions = {
            ext.lower() for ext in SubtitleFormatRegistry.enumerate_formats()
        }
        self.logger.info(
            "Supported subtitle formats: %s", ", ".join(sorted(supported_extensions))
        )

        files = self._discover_files(
            source_root, supported_extensions, destination_root
        )
        ...
        self.logger.info(
            "Translating %d subtitle file(s) from %s to %s",
            len(files),
            source_root,
            destination_root,
        )
        ...
        for index, source_file in enumerate(files, start=1):
            ...
            try:
                ...
                subtitles = init_subtitles(
                    filepath=str(source_file), options=self.options
                )
            ...
            try:
                ...
                destination_file = self._prepare_destination(
                    output_base, subtitles.file_format
                )

            except SubtitleError as exc:
                self.logger.error(
                    "Unable to determine output path for %s: %s", source_file, exc
                )
                stats.failed_files += 1
                continue
            ...
            try:
                ...
                translator: SubtitleTranslator = init_translator(
                    self.options, translation_provider=self.translation_provider
                )
            ...
            with self.progress_display.track(
                translator, source_file, translator.preview
            ):
                try:
                    ...
                    self.logger.exception(
                        "Unexpected error translating %s", source_file
                    )
                    stats.failed_files += 1
                    continue
            ...
            if translator.preview:
                self.logger.info(
                    "Preview mode enabled - skipping save for %s", source_file
                )
            ...
            try:
                ...
                raise SubtitleError(
                    f"Failed to save translation for {source_file}: {exc}"
                ) from exc

            if translator.errors:
                self.logger.warning(
                    "Translation completed with %d error(s) for %s",
                    len(translator.errors),
                    source_file,
                )
    ...
    def _initialise_provider(self) -> TranslationProvider:
        ...
        if not provider_name:
            raise SubtitleError(
                "No translation provider configured. Set the 'provider' option."
            )

        translation_provider = init_translation_provider(provider_name, self.options)

        self.logger.debug(
            "Validated translation provider %s", translation_provider.name
        )
...
def build_config(args: argparse.Namespace) -> BatchJobConfig:
    ...
    for override in args.option:
        if "=" not in override:
            raise SubtitleError(
                f"Invalid option override '{override}', expected KEY=VALUE"
            )
...
class ProgressDisplay:
    ...
    @contextmanager
    def track(
        self, translator: SubtitleTranslator, file_path: pathlib.Path, preview: bool
    ):
    ...
    def _on_batch_translated(self, _sender, batch) -> None:
        ...
        logging.info(
            "Translated batch %s: %s",
            self._last_batch_label,
            batch.summary or "no summary",
        )
        self._render()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Batch translate subtitles with PySubtrans"
    )
    parser.add_argument("source", nargs="?", help="Directory containing subtitle files")
    parser.add_argument(
        "destination", nargs="?", help="Directory to write translated subtitles"
    )
    ...
    parser.add_argument(
        "--language", dest="target_language", help="Target language for translation"
    )
    parser.add_argument(
        "--output-format",
        dest="output_format",
        help="Override the output subtitle format (e.g. srt)",
    )
    parser.add_argument(
        "--instructions",
        dest="instruction_file",
        help="Path to a file containing detailed instructions for the translator (system prompt)",
    )
    parser.add_argument(
        "--log-file", dest="log_file", help="Path to write the detailed log file"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Enable verbose console logging"
    )
    parser.add_argument(
        "--preview", dest="preview", action="store_true", help="Enable preview mode"
    )
    parser.add_argument(
        "--option",
        action="append",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        message = (
            ...
            if isinstance(error, SubtitleError)
            else str(error)
        )
