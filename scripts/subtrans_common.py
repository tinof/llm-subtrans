...
def InitLogger(logfilename: str, debug: bool = False) -> LoggerOptions:
    ...
    try:
        logging.basicConfig(
            format="%(levelname)s: %(message)s", encoding="utf-8", level=logging_level
        )
...
def CreateArgParser(description: str) -> ArgumentParser:
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output subtitle file path; format inferred from extension",
    )
    parser.add_argument(
        "--list-formats",
        action="store_true",
        help="List supported subtitle formats and exit",
    )
    parser.add_argument(
        "-l",
        "--target_language",
    ...
    parser.add_argument(
        "--names", type=str, default=None, help="A list of names to use verbatim"
    )
    parser.add_argument(
        "--postprocess",
        action="store_true",
    ...
    parser.add_argument(
        "--project",
        action="store_true",
        help="Create a persistent project file to allow resuming translation",
    )
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Create a project and preview the translation flow without calling the translation provider",
    )
    parser.add_argument(
        "--retranslate",
        action="store_true",
        help="Retranslate all subtitles, ignoring existing translations in the project file",
    )
    parser.add_argument(
        "--reparse",
        action="store_true",
        help="Reparse previous translation responses, reconstructing the translated subtitles",
    )
    parser.add_argument(
        "--reload",
        action="store_true",
        help="Reload the subtitles from the original file, ignoring existing subtitles in the project file",
    )
    parser.add_argument(
        "--ratelimit",
        type=int,
...
def CreateProject(options: Options, args: Namespace) -> SubtitleProject:
    ...
    if missing_params:
        raise ValueError(
            f"The following parameter(s) must be defined: {', '.join(missing_params)}"
        )
    ...
    logging.info(
        f"Created {scene_count} scenes and {batch_count} batches for translation"
    )

    if not args.output:
        output_path = GetOutputPath(
            project.subtitles.sourcepath,
            project.target_language or options.provider,
    ...
    logging.info(
        f"Translating {project.subtitles.linecount} subtitles from {project.subtitles.sourcepath}"
    )
