...
try:
    ...
    print(
        "Warning: python-dotenv not available; environment variables from .env file will not be loaded."
    )

# Model to use for auto-translation
free_translation_model = os.getenv(
    "FREE_TRANSLATION_MODEL", "qwen/qwen3-235b-a22b:free"
)  # Free but may be rate-limited or disappear
paid_translation_model = os.getenv(
    "PAID_TRANSLATION_MODEL", "google/gemini-2.5-flash"
)  # Fast and reliable but not free
...
def auto_translate_strings(
    untranslated: dict[str, str], target_language: str, paid: bool = False
) -> dict[str, str]:
    ...
    try:
        print(
            f"Calling OpenRouter API to translate {len(untranslated)} strings to {language_name}..."
        )
            ...
            try:
                ...
                if json_start != -1 and json_end > json_start:
                    ...
                    if isinstance(translations, dict):
                        ...
                        valid_translations = {
                            k: v for k, v in translations.items() if v and v.strip()
                        }
                        print(
                            f"Successfully translated {len(valid_translations)} strings"
                        )
...
def run_cmd(cmd: list[str]) -> None:
    try:
        ...
        print(
            f"ERROR: Skipping command due to missing tool: {' '.join(cmd)}",
            file=sys.stderr,
        )
        print(
            "       Please install gettext utilities to enable this functionality.",
            file=sys.stderr,
        )
...
def run_extract_strings() -> None:
    ...
    try:
        ...
        print(
            f"ERROR: extract_strings.py failed with exit code {e.returncode}",
            file=sys.stderr,
        )
...
def merge_and_compile(languages: list[str] | None = None):
    if not os.path.exists(POT_PATH):
        print(
            f"ERROR: POT not found at {POT_PATH}. Run extract_strings.py first.",
            file=sys.stderr,
        )
        return
    ...
    if not languages:
        print(
            "ERROR: No locales found. Please ensure locale directories exist in the locales folder.",
            file=sys.stderr,
        )
        return
    ...
    for lang in languages:
        ...
        try:
            ...
            print(
                f"WARNING: could not ensure header fields for {po_path}: {e}",
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                f"WARNING: could not fix newline parity for {po_path}: {e}",
                file=sys.stderr,
            )
        ...
        if stripped.startswith("msgid "):
            ...
            if i < len(lines) and lines[i].lstrip().startswith("msgstr "):
                ...
                if (
                    ends_id
                    and msgstr_first != ""
                    and (not ends_str_nl or ends_str_backslash_n)
                ):
...
def auto_translate_untranslated(
    untranslated_map: dict[str, dict[str, str]], paid: bool = False
) -> None:
    ...
    for lang, untranslated in untranslated_map.items():
        ...
        if translations:
            ...
            print(
                f"Saved {len(translations)} auto-translations to '{out_path}' for review."
            )
        else:
...
def _parse_translations_file(path: str) -> dict[str, str]:
    ...
    try:
        ...
        print(
            f"Warning: could not parse manual translations in {path}: {e}",
            file=sys.stderr,
        )
        return {}
...
def integrate_manual_translations(languages: list[str]) -> None:
    ...
    for lang in languages:
        ...
        if not translations:
            print(
                f"No manual translations found for {lang} (or none filled). Skipping."
            )
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="One-stop localization workflow for LLM-Subtrans"
    )
    parser.add_argument(
        "--auto",
        action="store_true",
    ...
    if auto_translate:
        ...
        if not api_key:
            print(
                "Error: --auto requires OPENROUTER_API_KEY environment variable to be set"
            )
            sys.exit(1)
    ...
    print(
        f"{steps.format(num=3)} Integrating any manual translations from untranslated_msgids_*.txt…"
    )
    ...
    if auto_translate:
        ...
        print(
            f"{steps.format(num=6)} Integrating auto-translations from autotranslated files…"
        )
