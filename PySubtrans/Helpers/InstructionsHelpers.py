...
from PySubtrans.Instructions import (
    Instructions,
    DEFAULT_TASK_TYPE,
    default_instructions,
    default_retry_instructions,
)
...
def LoadInstructionsFile(filepath: str) -> Instructions:
    ...
    if not lines[0].startswith("###"):
        ...
        if file_instructions:
            ...
            instructions.retry_instructions = (
                file_retry_instructions or default_retry_instructions
            )
    ...
    instructions.retry_instructions = (
        linesep.join(sections.get("retry_instructions", [])).strip()
        or default_retry_instructions
    )
    instructions.instruction_file = os.path.basename(filepath)
    instructions.target_language = (
        "".join(sections.get("target_language", []))
        if "target_language" in sections
        else None
    )
    instructions.task_type = (
        "".join(sections.get("task_type", []))
...
def GetInstructionsUserPath(instructions_file: str | None = None) -> str:
    ...
    return (
        os.path.join(instructions_dir, instructions_file)
        if instructions_file
        else instructions_dir
    )
...
def GetInstructionsFiles() -> list[str]:
    ...
    instructions_map = {
        os.path.basename(file).lower(): file for file in default_instructions
    }
    instructions_map.update(
        {os.path.basename(file).lower(): file for file in user_instructions}
    )
