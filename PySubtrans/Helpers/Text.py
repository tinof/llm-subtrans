...
standard_filler_words = (
    "um,umm,uh,uhh,er,err,ah,ahh,oh,eh,hm,hmm,hmmm,huh,ha,mmm,ow,oww"
)
...
def NormaliseDialogTags(text: str, dialog_marker: str) -> str:
    ...
    if any(part.startswith(dialog_marker) for part in line_parts):
        line_parts = [
            part if part.startswith(dialog_marker) else dialog_marker + part
...
def BreakLongLine(
    ...
    break_index = FindBreakPoint(
        text, break_sequences, max_line_length, min_line_length
    )
    if break_index:
...
def CompileFillerWordsPattern(
    filler_words: str | list[str],
) -> regex.Pattern[Any] | None:
    """
    ...
    filler_words_pattern = (
        rf"(^|[,¡¿]?\s+)({filler_pattern})({common_punctuation}+(\s+|$))"
    )

    return regex.compile(filler_words_pattern, flags=regex.IGNORECASE | regex.MULTILINE)


def RemoveFillerWords(
    text: str, fillerWords: str | list[str] | regex.Pattern[Any]
) -> str:
    """
    ...
    fillerPatterns = (
        fillerWords
        if isinstance(fillerWords, regex.Pattern)
        else CompileFillerWordsPattern(fillerWords)
    )
    ...
    def _append_previous_section(
        ...
        if previous_end > previous_start:
            if capitalise_first_letter:
                ...
                previous_section = (
                    first_letter
                    if next_index == previous_end
                    else first_letter + text[next_index:previous_end]
                )
    ...
    for match in fillerPatterns.finditer(text):
        ...
        _append_previous_section(
            output, text, last_index, start, capitalise_first_letter
        )
    ...
    if last_index < len(text):
        _append_previous_section(
            output, text, last_index, len(text), capitalise_first_letter
        )
...
def SanitiseSummary(
    summary: str, movie_name: str | None = None, max_summary_length: int | None = None
):
    """
    ...
    summary = regex.sub(
        r"^(?:(?:Scene|Batch)[\s\d:\-]*)+", "", summary, flags=regex.IGNORECASE
    )

    if movie_name:
        # Remove movie name and any connectors (-,: or whitespace)
        summary = regex.sub(
            r"^" + regex.escape(movie_name) + r"\s*[:\-\s]*", "", summary
        )
