...
from PySubtrans.Helpers.Time import (
    GetTimeDelta,
    GetTimeDeltaSafe,
...
class SubtitleLine:
    ...
    def __init__(
        self,
        line: SubtitleLine | str | dict | None = None,
        ...
        if isinstance(line, SubtitleLine):
            ...
            self.translation = (
                translation if translation is not None else line.translation
            )
            ...
            self.translation = (
                translation if translation is not None else line.get("translation")
            )
    ...
    def __str__(self) -> str:
        return (
            f"{self.number}:{self.srt_start}-->{self.srt_end}: {self.text}"
            if self._start is not None and self._end is not None
            else "Invalid SubtitleLine"
        )
    ...
    def duration(self) -> timedelta:
        if not self._duration:
            self._duration = (
                self.end - self.start
                if self._start is not None and self.end
    ...
    def translated(self) -> SubtitleLine | None:
        ...
        return SubtitleLine.Construct(
            self.number, self.start, self.end, self.translation, self.metadata
        )
    ...
    def duration(self, duration: timedelta | str):
        ...
        if isinstance(tdelta, Exception):
            raise SubtitleError("Invalid duration", error=tdelta)
    ...
    def _parse_from_string(self, line_str: str) -> None:
        ...
        if not line_str.strip():
            raise SubtitleError(
                _("Invalid subtitle line format: {}").format("empty line")
            )
        ...
        try:
            ...
            raise SubtitleError(
                _("Invalid subtitle line index: {}").format(match.group("index")),
                error=e,
            )
    ...
    def Construct(
        cls,
        number: int | str | None,
        start: timedelta | str | None,
        end: timedelta | str | None,
        text: str,
