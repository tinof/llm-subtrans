...
class SubtitleBatcher:
    def __init__(self, settings: SettingsType):
        ...
        self.max_batch_size: int = (
            settings.get_int("max_batch_size") or default_max_batch
        )
        self.fix_overlaps: bool = settings.get_bool("prevent_overlapping_times", False)

        scene_threshold_seconds: float = (
            settings.get_float("scene_threshold") or default_scene_threshold
        )
    ...
    def CreateNewScene(
        self, scenes: list[SubtitleScene], current_lines: list[SubtitleLine]
    ):
        """
