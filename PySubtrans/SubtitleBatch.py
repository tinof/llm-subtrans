...
class SubtitleBatch:
    def __init__(self, dct: dict[str, Any] | None = None):
        ...
        self._originals: list[SubtitleLine] = dct.get("originals", []) or dct.get(
            "subtitles", []
        )
    ...
    def error_messages(self) -> list[str]:
        ...
        return [
            str(error) for error in self.errors if isinstance(error, (str, Exception))
        ]
    ...
    def GetOriginalLine(self, line_number: int) -> SubtitleLine | None:
        ...
        return next(
            (line for line in self._originals if line.number == line_number), None
        )

    def GetTranslatedLine(self, line_number: int) -> SubtitleLine | None:
        """Get a translated line from the batch by its number"""
        return next(
            (line for line in self._translated if line.number == line_number), None
        )
    ...
    def PerformOutputSubstitutions(
        self, substitutions: Substitutions
    ) -> dict[str, str] | None:
        """
    ...
    def MergeLines(
        ...
        self._originals = (
            self.originals[:first_index] + [merged] + self.originals[last_index + 1 :]
        )

        translated_lines = [
            line for line in self.translated if line.number in line_numbers
        ]
    ...
    def DeleteLines(
        ...
        translated = [
            line for line in self.translated if line.number not in line_numbers
        ]

        if len(originals) == len(self.originals) and len(translated) == len(
            self.translated
        ):
            return [], []

        deleted_originals = [
            line for line in self.originals if line.number in line_numbers
        ]
        deleted_translated = [
            line for line in self.translated if line.number in line_numbers
        ]
    ...
    def InsertLines(
        ...
        translated: list[SubtitleLine] | None = None,
    ):
        """
