...
class SubtitleEditor:
    ...
    def __init__(
        self, subtitles: Subtitles, on_exit: Callable[[bool], None] | None = None
    ) -> None:
    ...
    def PreProcess(self, preprocessor: SubtitleProcessor) -> None:
        ...
        if self.subtitles.originals:
            self.subtitles.originals = preprocessor.PreprocessSubtitles(
                self.subtitles.originals
            )
    ...
    def UpdateBatch(
        self, scene_number: int, batch_number: int, update: dict[str, Any]
    ) -> bool:
    ...
    def UpdateLine(self, line_number: int, update: dict[str, Any]) -> bool:
        ...
        if not line:
            raise ValueError(
                f"Line {line_number} not found in batch ({batch.scene},{batch.number})"
            )
    ...
    ) -> list[tuple[int, int, list[SubtitleLine], list[SubtitleLine]]]:
        """
    ...
    def MergeScenes(self, scene_numbers: list[int]) -> SubtitleScene:
        ...
        if scene_numbers != list(
            range(scene_numbers[0], scene_numbers[0] + len(scene_numbers))
        ):
            raise ValueError("Scene numbers to be merged are not sequential")

        scenes: list[SubtitleScene] = [
            scene for scene in self.subtitles.scenes if scene.number in scene_numbers
        ]
        if len(scenes) != len(scene_numbers):
            raise ValueError(
                f"Could not find scenes {','.join([str(i) for i in scene_numbers])}"
            )
        ...
        self.subtitles.scenes = (
            self.subtitles.scenes[: start_index + 1]
            + self.subtitles.scenes[end_index + 1 :]
        )
    ...
    def MergeBatches(self, scene_number: int, batch_numbers: list[int]) -> None:
        ...
        scene: SubtitleScene | None = next(
            (scene for scene in self.subtitles.scenes if scene.number == scene_number),
            None,
        )
        if not scene:
    ...
    def MergeLinesInBatch(
        self, scene_number: int, batch_number: int, line_numbers: list[int]
    ) -> tuple[SubtitleLine, SubtitleLine | None]:
        """
    ...
    def SplitScene(self, scene_number: int, batch_number: int) -> None:
        ...
        if not batch:
            raise ValueError(
                f"Scene {scene_number} batch {batch_number} does not exist"
            )
        ...
        if split_index < len(self.subtitles.scenes):
            self.subtitles.scenes = (
                self.subtitles.scenes[:split_index]
                + [new_scene]
                + self.subtitles.scenes[split_index:]
            )
        else:
        ...
        for scene in self.subtitles.scenes:
            ...
            for batch in scene.batches:
                batch.originals = [
                    ...
                    if line.number and line.start is not None
                ]
                if batch.translated:
                    batch.translated = [
                        line
                ...
                unmatched_translated = [
                    ...
                    if line.number not in original_line_numbers
                ]
                if unmatched_translated:
                    logging.warning(
                        _(
                            "Removing {} translated lines in batch ({},{}) that don't match an original line"
                        ).format(len(unmatched_translated), batch.scene, batch.number)
                    )
                    batch.translated = [
                        line for line in translated if line not in unmatched_translated
                    ]

        self.subtitles.scenes = [
            scene for scene in self.subtitles.scenes if scene.batches
        ]
        self.RenumberScenes()
