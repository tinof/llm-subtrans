...
class ProviderSettingsView(MutableMapping[str, SettingsType]):
    ...
    def __init__(
        self, parent_dict: dict[str, SettingType], key: str = "provider_settings"
    ):
    ...
    def __getitem__(self, provider: str) -> SettingsType:
        ...
        if isinstance(settings, SettingsType):
            ...
            raise ValueError(
                f"Invalid provider settings type for {provider}: {type(settings)}"
            )

    def __setitem__(
        self, provider: str, settings: SettingsType | dict[str, Any]
    ) -> None:
        provider_dict = self._parent[self._key]
        if not isinstance(provider_dict, dict):
        ...
        if not isinstance(settings, SettingsType):
            if isinstance(settings, dict):
                ...
                raise ValueError(
                    f"Provider settings must be SettingsType or dict, got {type(settings)}"
                )
    ...
    def get_with_default(
        self, provider: str, default: SettingsType | None = None
    ) -> SettingsType | None:
