...
from PySubtrans.Helpers.ContextHelpers import (
    GetBatchContext,
    PrecomputeAllBatchContexts,
)
...
class SubtitleTranslator:
    ...
    def __init__(
        ...
        # Parallel batch translation settings
        self.parallel_batches = settings.get_bool("parallel_batches", False)
        self.max_parallel_workers = settings.get_int("max_parallel_workers", 4)
        ...
        try:
            self.client: TranslationClient = self.translation_provider.GetTranslationClient(
                self.settings
            )

        except Exception as e:
            raise ProviderError(
                _("Unable to create provider client: {error}").format(error=str(e)),
        ...
        self.postprocessor = (
            SubtitleProcessor(settings) if settings.get("postprocess_translation") else None
        )
    ...
    def TranslateSubtitles(self, subtitles: Subtitles):
        ...
        self._emit_info(
            _("Translating {linecount} lines in {scenecount} scenes").format(
                linecount=subtitles.linecount, scenecount=subtitles.scenecount
            )
        )
        ...
        if self.parallel_batches:
            self._translate_parallel(subtitles)
        else:
            self._translate_sequential(subtitles)
        ...
        if translations:
            self._emit_info(
                _("Successfully translated {count} lines!").format(count=len(translations))
            )
        ...
        if untranslated and not self.max_lines and not self.preview:
            ...
            for line in untranslated:
                self._emit_info(
                    _("Untranslated > {number}. {text}").format(number=line.number, text=line.text)
                )

        subtitles.originals = originals
        subtitles.translated = translations

    def _translate_sequential(self, subtitles: Subtitles):
        """
        Traditional sequential batch translation with inter-batch context.
        """
        for scene in subtitles.scenes:
            ...
            if self.resume and scene.all_translated:
                self._emit_info(
                    _("Scene {scene} already translated {linecount} lines...").format(
                        scene=scene.number, linecount=scene.linecount
                    )
                )
                continue
            ...
            batch_numbers = (
                [batch.number for batch in scene.batches if not batch.translated]
                if self.resume
                else None
            )
            ...
            if self.errors and self.stop_on_error:
                self._emit_error(
                    _("Failed to translate scene {scene}... stopping translation").format(
                        scene=scene.number
                    )
                )
                return

    def _translate_parallel(self, subtitles: Subtitles):
        """
        Parallel batch translation using pre-computed source-based context.
        All batches are translated concurrently using ThreadPoolExecutor.
        """
        self._emit_info(_("Using parallel batch translation mode"))

        # Pre-compute all context from source text
        max_history_tokens = self.settings.get_int("max_context_history_tokens", 10000)
        all_contexts = PrecomputeAllBatchContexts(subtitles, max_history_tokens)

        # Collect all batches to translate
        batches_to_translate: list[tuple[SubtitleScene, SubtitleBatch, dict[str, Any]]] = []
        ...
        for scene in subtitles.scenes:
            ...
            for batch in scene.batches:
                ...
                context = all_contexts.get((scene.number, batch.number), {})
                batches_to_translate.append((scene, batch, context))

        if not batches_to_translate:
            return

        self._emit_info(
            _("Translating {count} batches in parallel").format(count=len(batches_to_translate))
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_parallel_workers) as executor:
            ...
            future_to_batch = {
                executor.submit(self._translate_single_batch, batch, context): (
                    scene,
                    batch,
                )
                for scene, batch, context in batches_to_translate
            }
            ...
            for future in as_completed(future_to_batch):
                ...
                scene, batch = future_to_batch[future]
                try:
                    future.result()
                    # Emit batch translated event
                    self.events.batch_translated.send(self, batch=batch)
                except TranslationError as e:
                    self._emit_warning(
                        _("Error translating scene {scene} batch {batch}: {error}").format(
                            scene=batch.scene, batch=batch.number, error=str(e)
                        )
                    )
                    ...
                    self._emit_error(
                        _("Unexpected error translating batch {batch}: {error}").format(
                            batch=batch.number, error=str(e)
                        )
                    )
                    self.errors.append(TranslationError(str(e)))
                    if self.stop_on_error:
                        break

        # Emit scene_translated events for all scenes that had batches translated
        translated_scenes = set()
        for scene, batch, _ctx in batches_to_translate:
            if scene.number not in translated_scenes:
                self.events.scene_translated.send(self, scene=scene)
                translated_scenes.add(scene.number)

    def _translate_single_batch(self, batch: SubtitleBatch, context: dict[str, Any]):
        """
        Translate a single batch with pre-computed context.
        Thread-safe method for parallel translation.
        """
        if self.aborted:
            return
        ...
        batch.prompt = self.client.BuildTranslationPrompt(
            self.user_prompt, instructions, originals, context
        )

        if self.preview:
            return

        # For parallel mode, disable streaming to avoid complexity
        ...
        if translation and translation.reached_token_limit and not self.aborted:
            logging.warning(_("Hit API token limit, splitting batch and retrying..."))
            self._translate_split_batch(batch, originals, context, instructions)
            return

        if not self.aborted:
            if not translation:
                raise TranslationError(
                    f"Unable to translate scene {batch.scene} batch {batch.number}"
                )

            # Process the response
            self.ProcessBatchTranslation(batch, translation, None)

            # Consider retrying if there were errors
            if batch.errors and self.retry_on_error:
                logging.warning(
                    _(
                        "Scene {scene} batch {batch} failed validation, requesting retranslation"
                    ).format(scene=batch.scene, batch=batch.number)
                )
                self.RequestRetranslation(batch, line_numbers=None, context=context)
    ...
    def TranslateScene(
        ...
        try:
            batches = (
                [batch for batch in scene.batches if batch.number in batch_numbers]
                if batch_numbers
                else scene.batches
            )
            context = {}
            ...
            for batch in batches:
                ...
                if batch.errors:
                    self._emit_warning(
                        _("Errors encountered translating scene {scene} batch {batch}").format(
                ...
                if self.max_lines and self.lines_processed >= self.max_lines:
                    self._emit_info(
                        _("Reached max_lines limit of ({lines} lines)... finishing").format(
            ...
            scene.summary = self._get_best_summary(
                [scene.summary, context.get("scene"), context.get("summary")]
            )
    ...
    def TranslateBatch(
        ...
        if self.reparse and batch.translation:
            self._emit_info(
                _("Reparsing scene {scene} batch {batch} with {count} lines...").format(
                    scene=batch.scene, batch=batch.number, count=len(batch.originals)
                )
            )
        ...
        if self.resume and not self.retranslate and batch.all_translated:
            self._emit_info(
                _("Scene {scene} batch {batch} already translated {lines} lines...").format(
                    scene=batch.scene, batch=batch.number, lines=batch.size
                )
            )
            return
        ...
        streaming_callback = (
            self._create_streaming_callback(batch, line_numbers)
            if self.client.enable_streaming
            else None
        )
        translation: Translation | None = self.client.RequestTranslation(
            batch.prompt, streaming_callback=streaming_callback
        )

        if (translation and translation.reached_token_limit) and not self.aborted:
            logging.warning(_("Hit API token limit, splitting batch and retrying..."))
            self._translate_split_batch(
                batch, originals, context, instructions, line_numbers, streaming_callback
            )
            return
    ...
    def PreprocessBatch(
        self, batch: SubtitleBatch, context: dict[str, Any] | None = None
        ...
        if replacements:
            replaced: list[str] = [
                f"{Linearise(k)} -> {Linearise(v)}" for k, v in replacements.items()
            ]
            self._emit_info(
                _("Made substitutions in input:\n{replaced}").format(
                    replaced=linesep.join(replaced)
                )
            )
        ...
        with self.lock:
            line_count = (
                min(self.max_lines - self.lines_processed, len(originals))
                if self.max_lines
                else len(originals)
            )
    ...
    def ProcessBatchTranslation(
        ...
        if not translation.has_translation:
            raise TranslationError(
                "Translation contains no translated text", translation=translation
            )

        logging.debug(
            f"Scene {batch.scene} batch {batch.number} translation:\n{translation.text}\n"
        )
        ...
        batch.errors = [
            err for err in parser.errors if isinstance(err, str) or isinstance(err, SubtitleError)
        ]
        ...
        if batch.untranslated and not self.max_lines:
            self._emit_warning(
                _("Unable to match {count} lines with a source line").format(count=len(unmatched))
            )
            batch.AddContext(
                "untranslated_lines",
        ...
        if replacements:
            ...
            self._emit_info(
                _("Made substitutions in output:\n{replaced}").format(
                    replaced=linesep.join(replaced)
                )
            )
        ...
        if self.postprocessor:
            ...
            self._emit_info(
                _(
                    "Scene {scene} batch {batch}: {translated} lines and {untranslated} untranslated."
                ).format(
                    scene=batch.scene,
                    batch=batch.number,
    ...
    def RequestRetranslation(
        ...
        retranslation: Translation | None = self.client.RequestTranslation(
            prompt, retry_temperature
        )
        ...
        if not isinstance(retranslation, Translation):
            raise TranslationError(
                "Retranslation is not the expected type", translation=retranslation
            )

        logging.debug(
            f"Scene {batch.scene} batch {batch.number} retranslation:\n{retranslation.text}\n"
        )
        ...
        if batch.errors:
            self._emit_warning(
                _("Retry failed validation: {errors}").format(
                    errors=FormatErrorMessages(batch.errors)
                )
            )
        else:
    ...
    def _translate_split_batch(
        ...
        """
        Translate a batch by splitting it into two halves at the midpoint.
        Used as a fallback when the token limit is exceeded on the first attempt.
        The first half's context feeds into the second half for natural continuity.
        """
        mid = len(originals) // 2
        if mid == 0:
            logging.warning(_("Batch too small to split, cannot recover from token limit"))
            return

        logging.info(
            _("Splitting batch of {n} lines at midpoint for retranslation...").format(
                n=len(originals)
            )
        )

        first_half = originals[:mid]
        second_half = originals[mid:]

        # --- Translate first half ---
        batch_a = SubtitleBatch(
            {"scene": batch.scene, "number": batch.number, "originals": first_half}
        )
        batch_a.prompt = self.client.BuildTranslationPrompt(
            self.user_prompt, instructions, first_half, context
        )
        cb_a = (
            self._create_streaming_callback(batch_a, line_numbers)
            if streaming_callback is not None
            else None
        )
        translation_a = self.client.RequestTranslation(batch_a.prompt, streaming_callback=cb_a)

        if not translation_a or not translation_a.has_translation:
            raise TranslationError(
                f"Unable to translate first half of scene {batch.scene} batch {batch.number}"
            )

        self.ProcessBatchTranslation(batch_a, translation_a, line_numbers)

        # --- Build context from first half for second half ---
        context_b = dict(context)
        if translation_a.summary:
            context_b["summary"] = translation_a.summary
        if translation_a.scene:
            context_b["scene"] = translation_a.scene

        # --- Translate second half ---
        batch_b = SubtitleBatch(
            {"scene": batch.scene, "number": batch.number, "originals": second_half}
        )
        batch_b.prompt = self.client.BuildTranslationPrompt(
            self.user_prompt, instructions, second_half, context_b
        )
        cb_b = (
            self._create_streaming_callback(batch_b, line_numbers)
            if streaming_callback is not None
            else None
        )
        translation_b = self.client.RequestTranslation(batch_b.prompt, streaming_callback=cb_b)

        if not translation_b or not translation_b.has_translation:
            raise TranslationError(
                f"Unable to translate second half of scene {batch.scene} batch {batch.number}"
            )

        self.ProcessBatchTranslation(batch_b, translation_b, line_numbers)

        # --- Merge both halves back into the original batch ---
        batch._translated = MergeTranslations(batch_a.translated, batch_b.translated)
        batch.errors = batch_a.errors + batch_b.errors

        # Update context from the second half (most recent), falling back to first
        if not self.retranslate and not self.aborted:
            context["summary"] = self._get_best_summary(
                [translation_b.summary, translation_a.summary]
            )
            context["scene"] = self._get_best_summary(
                [translation_b.scene, translation_a.scene, context.get("scene")]
            )
            context["synopsis"] = (
                translation_b.synopsis or translation_a.synopsis or context.get("synopsis", "")
            )
            batch.UpdateContext(context)
        ...
        for candidate in candidates:
            ...
            if sanitised:
                if len(sanitised) < len(candidate):
                    self._emit_info(
                        _("Summary was truncated from {original} to {truncated} characters").format(
                            original=len(candidate), truncated=len(sanitised)
                        )
                    )
                return sanitised
    ...
    def _create_streaming_callback(
        self, batch: SubtitleBatch, line_numbers: list[int] | None
    ) -> StreamingCallback:
        """
        ...
        def streaming_callback(partial_translation: Translation):
            ...
            try:
                ...
                logging.warning(
                    f"Error processing streaming update for scene {batch.scene} batch {batch.number}: {e}"
                )

        return streaming_callback

    def _process_partial_translation(
        self,
        batch: SubtitleBatch,
        translation: Translation,
        line_numbers: list[int] | None,
