...
class OpenRouterProvider(TranslationProvider):
            ...
            SettingsType(
                {
                    ...
                    "server_address": settings.get_str(
                        "server_address",
                        os.getenv(
                            "OPENROUTER_SERVER_ADDRESS", "https://openrouter.ai/api/"
                        ),
                    ),
                    "stream_responses": settings.get_bool(
                        "stream_responses",
                        os.getenv("OPENROUTER_STREAM_RESPONSES", "True") == "True",
                    ),
                    "model_family": settings.get_str(
                        "model_family", os.getenv("OPENROUTER_MODEL_FAMILY", "Google")
                    ),
                    "only_translation_models": settings.get_bool(
                        "only_translation_models", True
                    ),
                    "model": settings.get_str(
                        "model", os.getenv("OPENROUTER_MODEL", "Gemini 2.5 Flash Lite")
                    ),
                    "max_tokens": settings.get_int(
                        "max_tokens", env_int("OPENROUTER_MAX_TOKENS", 0)
                    ),
                    "temperature": settings.get_float(
                        "temperature", env_float("OPENROUTER_TEMPERATURE", 0.0)
                    ),
                    "rate_limit": settings.get_float(
                        "rate_limit", env_float("OPENROUTER_RATE_LIMIT")
                    ),
    ...
    def available_model_families(self) -> list[str]:
        ...
        """
        available_families = (
            sorted(self._cached_models.keys()) if self._cached_models else []
        )
        valid_families: list[str] = [
            family for family in available_families if isinstance(family, str)
        ]
    ...
    def GetOptions(self, settings: SettingsType) -> GuiSettingsType:
        ...
        options = {
            "api_key": (
                str,
                _(
                    "An OpenRouter API key is required to use this provider (https://openrouter.ai/keys)"
                ),
            ),
            "use_default_model": (
                bool,
                _("Use the default model and hide advanced model options"),
            ),
        }
        ...
        if not settings.get_bool("use_default_model"):
            ...
            options["only_translation_models"] = (
                bool,
                _("Only show models from the translation category"),
            )
            ...
            if self._cached_models:
                options.update(
                    {
                        "model_family": (
                            ...
                            _("Model family/provider to choose from"),
                        ),
                    }
                )
                ...
                model_family: str | None = (
                    settings.get_str("model_family") or self.model_family
                )
                if model_family:
                    ...
                    if family_models:
                        ...
                        options["model"] = (
                            [_("No models available")],
                            _("Try a different model family or change filter settings"),
                        )
                else:
                    options["model"] = (
                        [_("No models available")],
            ...
            options.update(
                {
                    "stream_responses": (
                        bool,
                        _("Stream translations in realtime as they are generated"),
                    ),
                    "max_tokens": (
                        int,
                        _("Maximum number of output tokens to return in the response."),
                    ),
                    "temperature": (
                        float,
                        _(
    ...
    def _populate_model_cache(self):
        ...
        if self._cached_models and self._model_cache_filtered == self.settings.get_bool(
            "only_translation_models", True
        ):
        ...
        try:
            ...
            with httpx.Client(timeout=20) as client:
                ...
                if result.is_error:
                    logging.error(
                        _("Error fetching models: {status} {text}").format(
                ...
                try:
                    ...
                    self._all_model_list = sorted(
                        self._get_model_name(model)[1]
                    ...
                    logging.error(
                        _(
                            "Unable to parse server response as JSON: {response_text}"
                        ).format(response_text=result.text)
                    )
                    return

        except Exception as e:
            logging.error(
                _("Unable to retrieve available models: {error}").format(error=str(e))
            )
            return
