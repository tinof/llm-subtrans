...
class CustomClient(TranslationClient):
    ...
    def __init__(self, settings: SettingsType):
        ...
        self._emit_info(
            _("Translating with server at {server_address}{endpoint}").format(
    ...
    def _make_request(
        ...
        for retry in range(self.max_retries + 1):
            ...
            try:
                ...
                if self.server_address is None or self.endpoint is None:
                    raise TranslationImpossibleError(
                        _("Server address or endpoint is not set")
                    )
                ...
                if not self.aborted:
                    self._emit_error(
                        _(
                            "Failed to connect to server at {server_address}{endpoint}"
                        ).format(
                            server_address=self.server_address, endpoint=self.endpoint
                        )
                    )
                ...
                if not self.aborted:
                    self._emit_error(
                        _("Network error communicating with server: {error}").format(
                            error=str(e)
                        )
                    )
                ...
                if not self.aborted:
                    self._emit_error(
                        _("Request to server timed out: {error}").format(error=str(e))
                    )

            except Exception as e:
                raise TranslationImpossibleError(
                    _("Unexpected error communicating with server"), error=e
                )
            ...
            if retry == self.max_retries:
                raise TranslationImpossibleError(
                    _(
                        "Failed to communicate with server after {max_retries} retries"
                    ).format(max_retries=self.max_retries)
                )

            sleep_time = self.backoff_time * 2.0**retry
            self._emit_warning(
                _("Retrying in {sleep_time} seconds...").format(sleep_time=sleep_time)
            )
        ...
        if result.is_error:
            ...
            if result.is_client_error:
                raise TranslationResponseError(
                    _("Client error: {status_code} {text}").format(
                ...
                raise TranslationResponseError(
                    _("Server error: {status_code} {text}").format(
                        status_code=result.status_code, text=summary_text
                    ),
                    response=result,
                )
    ...
    def _handle_streaming_request(
        self, request: TranslationRequest, request_body: dict[str, Any]
    ) -> dict[str, Any] | None:
        ...
        with self.client.stream("POST", self.endpoint, json=request_body) as response:
            ...
            try:
                for line in response.iter_lines():
                    ...
                    if chunk_data:
                        ...
                        if chunk_data.get("error"):
                            ...
                            raise TranslationResponseError(
                                _("Streaming error: {error}").format(error=error_msg),
                                response=response,
                            )
                        ...
                        self._process_streaming_chunk(
                            request, chunk_data, accumulated_response
                        )
                ...
                if chunks_processed == 0:
                    ...
                    raise TranslationImpossibleError(
                        _("Failed to establish streaming connection: {error}").format(
                            error=str(e)
                        )
                    )
                else:
                    ...
                    self._emit_warning(
                        f"Streaming connection interrupted after {chunks_processed} chunks: {e}"
                    )
    ...
    def _parse_sse_chunk(self, line: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            self._emit_warning(
                f"Failed to parse SSE data chunk as JSON: {data_part[:100]}... Error: {e}"
            )
            return None
        ...
        if "usage" in chunk_data:
            ...
            if isinstance(usage, dict):
                ...
                if "reasoning_tokens" in usage:
                    accumulated_response["reasoning_tokens"] = usage.get(
                        "reasoning_tokens"
                    )
    ...
    def _process_api_response(
        self, content: dict[str, Any], result: httpx.Response
    ) -> dict[str, Any]:
        ...
        if not response.get("text"):
            raise TranslationResponseError(
                _("No text returned in the response"), response=result
            )
    ...
    def _add_additional_headers(self, settings):
        additional_headers = settings.get(
            "additional_headers", {}
        )  # Keep dict access for complex types
