...
class OpenAIReasoningClient(OpenAIClient):
    ...
    def _send_messages(
        self, request: TranslationRequest, temperature: float | None
    ) -> dict[str, Any] | None:
        """
    ...
    def _handle_streaming_response(
        ...
        if not isinstance(request.prompt.content, list):
            raise TranslationImpossibleError(
                _("Content must be a list for streaming Responses API")
            )
        ...
        try:
            for event in stream:
                ...
                if isinstance(event, ResponseTextDeltaEvent):
                    ...
                    self._emit_warning(
                        _(
                            "Streaming ended with an error but OpenAI returned a response: {error}"
                        ).format(
        ...
        if stream_error:
            ...
            raise TranslationError(
                _("Streaming failed before any response was received: {error}").format(
                    error=str(stream_error)
                )
            )
        ...
        raise TranslationResponseError(
            _("OpenAI streaming ended without a completed response"), response=None
        )

    def _convert_to_input_params(
        self, content: str | list[str] | list[dict[str, str]]
    ) -> ResponseInputParam:
        """
        ...
        if not isinstance(content, list):
            raise TranslationImpossibleError(
                _("Content must be a list for Responses API")
            )

        # Content should always be a list of dictionaries for this client
        if content and isinstance(content[0], str):
            raise TranslationImpossibleError(
                _("Content must be a list of message dicts for Responses API")
            )
        ...
        for msg in content:
            if not isinstance(msg, dict):
                raise TranslationImpossibleError(
                    _(
                        "Content must be a list of message dicts for Responses API. Found item of type {type}."
                    ).format(type=type(msg).__name__)
                )
            ...
            if role not in ("user", "system", "developer", "assistant"):
                raise TranslationImpossibleError(
                    _(
                        "Invalid message role: {role}. Must be one of 'user', 'system', 'developer', or 'assistant'."
                    ).format(role=role)
                )
            ...
            input_params.append(
                EasyInputMessageParam(
                    content=msg_content, role=typed_role, type="message"
                )
            )
    ...
    def _extract_text_content(self, openai_response: responses_types.Response):
                ...
                for content_item in content:
                    ...
                    if isinstance(reasoning_value, list):
                        for reasoning_part in reasoning_value:
                            ...
                            elif hasattr(reasoning_part, "text") and getattr(
                                reasoning_part, "text"
                            ):
        ...
        raise TranslationResponseError(
            _("No text content found in response"), response=openai_response
        )

    def _extract_usage_info(
        self, openai_response: responses_types.Response
    ) -> dict[str, Any]:
        ...
        if isinstance(legacy_details, CompletionTokensDetails):
            ...
            if legacy_details.accepted_prediction_tokens is not None:
                info["accepted_prediction_tokens"] = (
                    legacy_details.accepted_prediction_tokens
                )
            if legacy_details.rejected_prediction_tokens is not None:
                info["rejected_prediction_tokens"] = (
                    legacy_details.rejected_prediction_tokens
                )
    ...
    def _normalize_finish_reason(self, result):
        ...
        finish = getattr(result, "stop_reason", None) or getattr(
    ...
    def _raise_for_openai_error(self, error: OpenAIError) -> None:
        ...
        if isinstance(error, BadRequestError):
            if (
                "reasoning.effort" in error_message
                or "reasoning" in error_message.lower()
            ):
                raise TranslationImpossibleError(
                    _(
                        "Unsupported reasoning effort: {error}. Choose a different reasoning effort or model."
                    ).format(error=error_message)
                ) from error
            else:
                raise TranslationImpossibleError(
                    _("API could not process the request: {error}.").format(
                        error=error_message
                    )
                ) from error
        ...
        if isinstance(error, AuthenticationError):
            raise TranslationImpossibleError(
                _("Authentication failed: {error}").format(error=error_message)
            ) from error

        if isinstance(error, PermissionDeniedError):
            raise TranslationImpossibleError(
                _("Permission denied: {error}").format(error=error_message)
            ) from error

        if isinstance(error, NotFoundError):
            raise TranslationImpossibleError(
                _(
                    "Requested resource not found: {error}. '{model}' may not be available for your account."
                ).format(error=error_message, model=self.model or "unknown")
            ) from error
