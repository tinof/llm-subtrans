            ...
            SettingsType(
                {
                    ...
                    "endpoint": settings.get_str(
                        "endpoint", os.getenv("CUSTOM_ENDPOINT", "/v1/chat/completions")
                    ),
                    "supports_conversation": settings.get_bool(
                        "supports_conversation",
                        env_bool("CUSTOM_SUPPORTS_CONVERSATION", True),
                    ),
                    "supports_system_messages": settings.get_bool(
                        "supports_system_messages",
                        env_bool("CUSTOM_SUPPORTS_SYSTEM_MESSAGES", True),
                    ),
                    "prompt_template": settings.get_str(
                        "prompt_template",
                        os.getenv("CUSTOM_PROMPT_TEMPLATE", default_prompt_template),
                    ),
                    "temperature": settings.get_float(
                        "temperature", env_float("CUSTOM_TEMPERATURE", 0.0)
                    ),
                    "max_tokens": settings.get_int(
                        "max_tokens", env_int("CUSTOM_MAX_TOKENS", 0)
                    ),
                    "max_completion_tokens": settings.get_int(
                        "max_completion_tokens",
                        env_int("CUSTOM_MAX_COMPLETION_TOKENS", 0),
                    ),
                    "timeout": settings.get_int(
                        "timeout", env_int("CUSTOM_TIMEOUT", 300)
                    ),
                    "api_key": settings.get_str("api_key", os.getenv("CUSTOM_API_KEY")),
                    "model": settings.get_str("model", os.getenv("CUSTOM_MODEL")),
                    "supports_parallel_threads": settings.get_bool(
                        "supports_parallel_threads",
                        env_bool("CUSTOM_SUPPORTS_PARALLEL_THREADS", False),
                    ),
                }
            ),
        )
    ...
    def GetOptions(self, settings: SettingsType) -> GuiSettingsType:
        ...
        if self.ValidateSettings():
            options["supports_conversation"] = (
                bool,
                _("Attempt to communicate with the endpoint using chat format"),
            )
            ...
            options.update(
                {
                    ...
                    "max_completion_tokens": (
                        int,
                        _("Alternative to max_tokens for some servers"),
                    ),
                    "timeout": (
                        int,
                        _("Timeout for the request in seconds (default 300)"),
                    ),
                    "api_key": (
                        str,
                        _(
                            "API key if needed (this is normally not needed for a local server)"
                        ),
                    ),
                    "model": (
                        str,
                        _(
                            "The model ID (for local servers this is usually not required"
                        ),
                    ),
                    "supports_parallel_threads": (
                        bool,
