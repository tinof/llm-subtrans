...
class SSAFileHandler(SubtitleFileHandler):
    ...
    def load_file(self, path: str) -> SubtitleData:
        ...
        try:
            subs: pysubs2.SSAFile = pysubs2.SSAFile.load(
                path, encoding=default_encoding, format_=format_hint
            )
            return self._parse_subs(subs)
        except UnicodeDecodeError:
            subs: pysubs2.SSAFile = pysubs2.SSAFile.load(
                path, encoding=fallback_encoding, format_=format_hint, newline=""
            )
            return self._parse_subs(subs)
            ...
            raise (
                ...
                else SubtitleParseError(
                    _("Failed to parse subtitles: {}").format(str(e)), e
                )
            )
    ...
    def _parse_subs(self, subs: pysubs2.SSAFile):
        ...
        try:
            ...
            return SubtitleData(
                lines=lines, metadata=metadata, detected_format=detected_format
            )
    ...
    def _pysubs2_to_subtitle_line(
        self, pysubs2_line: pysubs2.SSAEvent, index: int
    ) -> SubtitleLine:
        ...
        return SubtitleLine.Construct(
            number=index, start=start, end=end, text=text, metadata=metadata
        )
    ...
    def _parse_metadata(self, subs: pysubs2.SSAFile, subtitle_format: str) -> dict:
        ...
        metadata = {
            ...
            if hasattr(subs, "aegisub_project")
            else {},
        }
            ...
            for style_name, style_fields in metadata["styles"].items():
                ...
                for field, value in style_data.items():
                    if isinstance(value, Color):
                        style_data[field] = pysubs2.Color(
    ...
    def _ssa_to_html(self, ssa_text: str) -> str:
        ...
        for tag_match in _TAG_BLOCK_PATTERN.finditer(text):
            ...
            if _STANDALONE_BASIC_TAG_PATTERN.match(block):
                ...
                basic_tags = [
                    "{" + m.group(0) + "}" for m in _BASIC_TAG_PATTERN.finditer(block)
                ]
