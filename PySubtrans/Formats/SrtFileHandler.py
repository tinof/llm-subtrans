    ...
    def compose(self, data: SubtitleData) -> str:
        ...
        if len(output_lines) < len(data.lines):
            ...
            if num_invalid:
                logging.warning(
                    _(
                        "{} lines were invalid and were not written to the output file"
                    ).format(num_invalid)
                )
            ...
            if num_empty:
                logging.warning(
                    _(
                        "{} lines were empty and were not written to the output file"
                    ).format(num_empty)
                )
        ...
        if data.metadata.get("add_rtl_markers"):
            for line in output_lines:
                if (
                    line.text
                    and IsRightToLeftText(line.text)
    ...
    def _parse_srt_items(self, source) -> Iterator[SubtitleLine]:
        ...
        try:
            ...
            raise SubtitleParseError(
                _("Unexpected error parsing SRT: {}").format(str(e)), e
            )
