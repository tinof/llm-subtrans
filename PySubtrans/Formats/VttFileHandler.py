...
class VttFileHandler(SubtitleFileHandler):
    ...
    _VOICE_TAG_PATTERN = regex.compile(
        r"^\s*<v((?:\.[\w-]+)*)(?:\s+([^>]+))?>((?:(?!</?v).)*)</v>\s*$"
    )
        ...
        try:
            ...
            if not lines or not lines[0].strip().lstrip("\ufeff").startswith("WEBVTT"):
                raise SubtitleParseError(
                    _("Invalid WebVTT file: missing WEBVTT header")
                )
            ...
            return SubtitleData(
                lines=subtitle_lines, metadata=file_metadata, detected_format=".vtt"
            )
    ...
    def compose(self, data: SubtitleData) -> str:
        ...
        for line in data.lines:
            if line.text and line.start is not None and line.end is not None:
                ...
                output_text = self._restore_vtt_text(
                    line.text or "", line.metadata or {}
                )
    ...
    def _is_content_line(self, line: str) -> bool:
        ...
        return bool(
            self._TIMESTAMP_PATTERN.match(line)
            or self._STYLE_BLOCK_START.match(line)
            or self._NOTE_BLOCK_START.match(line)
        )
    ...
    def _parse_single_cue(
        self, lines: list[str], start_idx: int, line_number: int
    ) -> tuple[SubtitleLine | None, int]:
        ...
        timestamp_match = self._TIMESTAMP_PATTERN.match(
        ...
        cue_settings = (
            timestamp_match.group(9).strip() if timestamp_match.group(9) else ""
        )
    ...
    def _process_vtt_text(self, text: str) -> tuple[str, dict]:
        ...
        if voice_match:
            ...
            if css_classes:
                classes = (
                    css_classes[1:].split(".") if css_classes.startswith(".") else []
                )
