...
SettingType: TypeAlias = (
    BasicType | dict[str, "SettingType"] | dict[str, "SettingsType"]
)
...
class SettingsType(dict[str, SettingType]):
    ...
    def get_bool(self, key: str, default: bool | None = False) -> bool:
        ...
        raise SettingsError(
            f"Cannot convert setting '{key}' of type {type(value).__name__} with value {repr(value)} to bool"
        )
    ...
    def get_int(self, key: str, default: int | None = None) -> int | None:
        ...
        raise SettingsError(
            f"Cannot convert setting '{key}' of type {type(value).__name__} with value {repr(value)} to int"
        )
    ...
    def get_float(self, key: str, default: float | None = None) -> float | None:
        ...
        raise SettingsError(
            f"Cannot convert setting '{key}' of type {type(value).__name__} with value {repr(value)} to float"
        )
    ...
    def get_timedelta(self, key: str, default: timedelta) -> timedelta:
        ...
        raise SettingsError(
            f"Cannot convert setting '{key}' of type {type(value).__name__} to timedelta"
        )
    ...
    def get_list(self, key: str, default: list[Any] | None = None) -> list[Any]:
        ...
        raise SettingsError(
            f"Cannot convert setting '{key}' of type {type(value).__name__} to list"
        )
...
def redact_sensitive_values(settings: SettingsType) -> SettingsType:
    ...
    for key, value in settings.items():
        if isinstance(value, SettingsType):
            ...
            s in key.lower()
            for s in ("key", "token", "password", "secret", "auth", "credential")
        ):
            redacted[key] = "***"
        else:
            redacted[key] = value
