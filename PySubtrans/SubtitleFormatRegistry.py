...
class SubtitleFormatRegistry:
    ...
    def get_handler_by_extension(cls, extension: str) -> type[SubtitleFileHandler]:
        ...
        if ext not in cls._handlers:
            raise ValueError(
                _(
                    "Unknown subtitle format: {extension}. Available formats: {available}"
                ).format(extension=extension, available=cls.list_available_formats())
            )
        return cls._handlers[ext]

    @classmethod
    def create_handler(
        cls, extension: str | None = None, filename: str | None = None
    ) -> SubtitleFileHandler:
        """
        ...
        if extension is None or not extension:
            raise ValueError(
                _(
                    ...
                    name=filename or extension or "None",
                    formats=cls.list_available_formats(),
                )
            )
    ...
    def detect_format_from_content(cls, content: str) -> str | None:
        ...
        try:
            ...
            logging.info(
                _("Detected subtitle format '{format}' from content").format(
            ...
            raise SubtitleParseError(
                _("Failed to detect subtitle format: {}").format(str(e)), e
            )
    ...
    def detect_format_and_load_file(cls, path: str) -> SubtitleData:
        ...
        if not subs.format:
            raise SubtitleParseError(
                _("Could not detect subtitle format for file: {}").format(path)
            )

        detected_extension = pysubs2.formats.get_file_extension(subs.format)

        logging.info(
            _("Detected subtitle format '{format}'").format(format=detected_extension)
        )

        if detected_extension not in cls._handlers:
            raise SubtitleParseError(
                _("Detected subtitle format '{format}' is not supported.").format(
                    format=detected_extension
                )
            )
