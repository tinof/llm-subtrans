...
class Instructions:
    ...
    def InitialiseInstructions(self, settings: dict[str, str | None]) -> None:
        ...
        self.retry_instructions = (
            settings.get("retry_instructions") or default_retry_instructions
        )
        ...
        if settings.get("instruction_args") and isinstance(
            settings["instruction_args"], list
        ):
            additional_instructions = linesep.join(settings["instruction_args"])
            if additional_instructions:
                self.instructions = linesep.join(
                    [self.instructions, additional_instructions]
                )

        tags = {
            "[ for movie]": f" for {settings.get('movie_name')}"
            if settings.get("movie_name")
            else "",
            "[ to language]": f" to {settings.get('to_language')}"
            if settings.get("to_language")
            else "",
        }
