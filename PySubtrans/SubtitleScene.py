...
class SubtitleScene:
    ...
    def originals(self) -> list[SubtitleLine] | None:
        ...
        return (
            [line for batch in self.batches for line in batch.originals]
            if self.batches
            else None
        )
    ...
    def translated(self) -> list[SubtitleLine] | None:
        ...
        return (
            [line for batch in self.batches for line in batch.translated]
            if self.batches
            else None
        )
    ...
    def batches(self, value: list[SubtitleBatch]):
        if not isinstance(value, list) or not all(
            isinstance(v, SubtitleBatch) for v in value
        ):
    ...
    def MergeBatches(self, batch_numbers: list[int]):
        ...
        if batch_numbers != list(
            range(batch_numbers[0], batch_numbers[0] + len(batch_numbers))
        ):
        ...
        if len(batches) != len(batch_numbers):
            raise ValueError(
                f"Could not find batches {str(batch_numbers)} in scene {self.number}"
            )
        ...
        merged_batch.summary = "\n".join(
            batch.summary for batch in batches if batch.summary
        )
        ...
        self._batches = (
            self._batches[:start_index]
            + [merged_batch]
            + self._batches[end_index + 1 :]
        )

        self._renumber_batches()

    def SplitBatch(
        self, batch_number: int, line_number: int, translated_number: int | None = None
    ):
        ...
        if batch.translated:
            ...
            translated_index = next(
                (
                    i
                    for i, line in enumerate(batch.translated)
                    if line.number == split_translated
                ),
                -1,
            )
            if translated_index >= 0:
                ...
                logging.warning(
                    f"Translated line number {translated_number} not found in batch translations"
                )
