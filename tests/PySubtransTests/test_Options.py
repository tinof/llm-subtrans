...
class TestOptions(LoggedTestCase):
    ...
    def test_initialization_with_options_object(self):
        ...
        self.assertLoggedEqual(
            "original.get('provider') (original unchanged)",
            "Test Provider",
            original_result,
        )

        self.assertLoggedEqual(
            "copy_options.get('provider') (copy modified)",
            "Different Provider",
            copy_result,
        )
    ...
    def test_initialization_with_kwargs(self):
        ...
        options = Options(
            provider="Kwargs Provider", target_language="French", max_batch_size=50
        )
    ...
    def test_initialization_dict_and_kwargs(self):
        ...
        options = Options(
            self.test_options, provider="Kwargs Override Provider", max_batch_size=100
        )
    ...
    def test_none_values_filtered(self):
        ...
        self.assertLoggedIsNone(
            "options.get('custom_setting') (None custom setting filtered)",
            custom_result,
        )
    ...
    def test_get_method(self):
        ...
        self.assertLoggedEqual(
            "options.get('non_existing', 'default') (with default)",
            "default",
            default_result,
        )
    ...
    def test_properties(self):
        ...
        options = Options(
            {
                ...
                "provider": "Test Provider",
                "provider_settings": {
                    "Test Provider": {"model": "test-model", "api_key": "test-key"}
                },
        ...
        if options.current_provider_settings is not None:
            self.assertEqual(
                options.current_provider_settings.get_str("model"), "test-model"
            )
    ...
    def test_current_provider_settings_missing_provider(self):
        ...
        options = Options({"provider": "Missing Provider", "provider_settings": {}})
        self.assertIsNone(options.current_provider_settings)
    ...
    def test_model_property(self):
        ...
        options = Options({"provider": "Test Provider", "provider_settings": {}})
        self.assertIsNone(options.model)
        ...
        options = Options(
            {
                "provider": "Test Provider",
                "provider_settings": {"Test Provider": {"model": "test-model"}},
            }
        )
    ...
    def test_load_settings_success(
        self, mock_exists, mock_file, mock_json_load, mock_version
    ):
    ...
    def test_build_user_prompt(self):
        ...
        expected = (
            "Translate to Spanish for Test Movie the following subtitles: test_value"
        )
    ...
    def test_build_user_prompt_empty_values(self):
        ...
        result = options.BuildUserPrompt()
        expected = (
            "Translate to English subtitles"  # target_language defaults to English
        )
        self.assertEqual(result, expected)
    ...
    def test_version_update_migration(self):
        ...
        options = Options(
            {"gpt_model": "old-model-name", "api_key": "test-key", "version": "v0.9.0"}
        )
...
class TestSettingsType(LoggedTestCase):
        ...
        self.test_settings = SettingsType(
            {
                ...
                "nested_dict": SettingsType(
                    {"inner_str": "nested_value", "inner_int": 100, "inner_bool": True}
                ),
                "none_value": None,
            }
        )
    ...
    def test_get_str_list(self):
        ...
        self.assertLoggedSequenceEqual(
            "get_str_list with custom default", custom_default, result
        )
    ...
    def test_get_dict(self):
        ...
        self.assertLoggedEqual(
            "nested dict update propagated", "new_value", updated_nested["new_key"]
        )
        ...
        if isinstance(direct_nested, SettingsType):
            ...
            self.assertLoggedEqual(
                "nested update visible in direct access",
                "new_value",
                direct_nested["new_key"],
            )
    ...
    def test_provider_settings_nested_updates(self):
        ...
        options = Options(
            {
                ...
                "provider_settings": {
                    ...
                    "Other Provider": SettingsType(
                        {"model": "other-model", "temperature": 0.5}
                    ),
                },
            }
        )
        ...
        self.assertLoggedIsInstance(
            "provider_settings is MutableMapping", provider_settings, MutableMapping
        )

        # Test accessing existing provider settings
        test_provider_settings = provider_settings["Test Provider"]
        self.assertLoggedIsInstance(
            "provider settings is SettingsType", test_provider_settings, SettingsType
        )
        ...
        self.assertLoggedEqual(
            "nested provider update propagated",
            "new_value",
            updated_provider_settings["new_setting"],
        )

        # Test adding a new provider through the mutable mapping
        new_provider_settings = SettingsType(
            {"model": "new-provider-model", "temperature": 0.8}
        )
        ...
        self.assertLoggedEqual(
            "new provider model", "new-provider-model", new_settings.get_str("model")
        )
        ...
        if current_settings:
            ...
            self.assertLoggedEqual(
                "current provider model", "test-model", current_model
            )
            ...
            if provider is not None:
                ...
                self.assertLoggedEqual(
                    "current provider update propagated",
                    "current_value",
                    updated_current["current_test"],
                )
...
class TestSettingsHelpers(LoggedTestCase):
    ...
    def test_get_timedelta_setting(self):
        ...
        for key, expected in test_cases:
            with self.subTest(key=key):
                default = timedelta(
                    minutes=1
                )  # Use a different default to distinguish from expected
    ...
    def test_get_timedelta_setting_errors(self):
        ...
        with self.assertRaises(SettingsError) as cm:
            self.test_dict_settings.get_timedelta(
                "timedelta_invalid", timedelta(seconds=1)
            )
    ...
    def test_get_optional_setting(self):
        ...
        for key, setting_type, expected in test_cases:
            with self.subTest(key=key):
                result = get_optional_setting(
                    self.test_dict_settings, key, setting_type
                )
                self.assertLoggedEqual(key, expected, result)
    ...
    def test_validate_setting_type(self):
        ...
        for key, setting_type, expected in valid_cases:
            with self.subTest(key=key):
                result = validate_setting_type(
                    self.test_dict_settings, key, setting_type
                )
                self.assertLoggedEqual(
                    f"'{key}' as {setting_type.__name__}", expected, result
                )
    ...
    def test_validate_setting_type_errors(self):
        ...
        result = validate_setting_type(
            self.test_dict_settings, "missing_key", str, required=False
        )
        self.assertLoggedTrue("missing optional setting", result)

        # Test invalid type
        result = validate_setting_type(
            self.test_dict_settings, "bool_str_invalid", bool
        )
        ...
        with self.assertRaises(SettingsError) as cm:
            validate_setting_type(
                self.test_dict_settings, "missing_key", str, required=True
            )
...
def get_optional_setting(
    settings: SettingsType | Mapping[str, SettingType], key: str, setting_type: type[T]
) -> T | None:
    """
    ...
    if setting_type == bool:
        ...
        if isinstance(value, setting_type):
            ...
            raise SettingsError(
                f"Cannot convert setting '{key}' of type {type(value).__name__} to {setting_type.__name__}"
            )


def validate_setting_type(
    settings: SettingsType | Mapping[str, SettingType],
    key: str,
