...
"""
    ...
    def test_load_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            "w", delete=False, suffix=".vtt", encoding="utf-8"
        ) as f:
    ...
    def test_composition_variations(self):
        ...
        test_cases = [
            ...
            },
            {
                ...
                "metadata": {},
                "should_contain": [
                    "position:10% align:left size:35%",
                    "Where did he go?",
                ],
            },
            {
    ...
    def test_round_trip_conversion(self):
        ...
        self.assertLoggedEqual(
            "Original lines", len(original_lines), len(round_trip_lines)
        )

        # Validate metadata preservation
        self.assertLoggedEqual(
            "Metadata format preserved",
            original_data.detected_format,
            round_trip_data.detected_format,
        )
...
"""
        ...
        self.assertLoggedEqual(
            "First cue settings",
            "position:10%,line-left align:left size:35%",
            first_settings,
        )
        ...
        self.assertLoggedEqual(
            "Second cue settings", "position:90% align:right size:35%", second_settings
        )
        ...
        self.assertLoggedEqual(
            "Round-trip settings preserved",
            first_settings,
            round_trip_lines[0].metadata.get("vtt_settings"),
        )
        self.assertEqual(
            round_trip_lines[1].metadata.get("vtt_settings"), second_settings
        )
...
"""
    ...
    voice_metadata_cases = {
        ...
        "<v.first-second Mary>Text</v>": {
            "voice_classes": ["first-second"],
            "speaker": "Mary",
        },
        "<v.class1.class2 John>Multiple</v>": {
            "voice_classes": ["class1", "class2"],
            "speaker": "John",
        },
        "<v>No attrs</v>": {},
...
"""
        ...
        self.assertLoggedNotIn(
            "No tag duplication", "<v.first-second Mary><v.first-second Mary>", composed
        )
