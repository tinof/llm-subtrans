...
class TestSubtitleFormatRegistry(LoggedTestCase):
    ...
    def test_CreateHandlerWithFilename(self):
        ...
        self.assertLoggedIsInstance(
            "test.srt handler instance", handler, SrtFileHandler
        )
    ...
    def test_DetectFormatAndLoadFile(self):
        ...
        try:
            ...
            self.assertLoggedIn(
                "metadata has detected_format", "detected_format", data.metadata
            )
...
"""
    ...
    def test_FormatDetectionWithMalformedFile(self):
        ...
        try:
            ...
            log_input_expected_error(
                "malformed file content", SubtitleParseError, e.exception
            )
    ...
    def test_FormatDetectionWithEmptyFile(self):
        ...
        try:
            with self.assertRaises(SubtitleParseError) as e:
                SubtitleFormatRegistry.detect_format_and_load_file(temp_path)
            log_input_expected_error(
                "empty file content", SubtitleParseError, e.exception
            )
        finally:
            os.unlink(temp_path)
    ...
    def test_FormatDetectionWithBinaryFile(self):
        ...
        try:
            with self.assertRaises(SubtitleParseError) as e:
                SubtitleFormatRegistry.detect_format_and_load_file(temp_path)
            log_input_expected_error(
                "binary file content", SubtitleParseError, e.exception
            )
        finally:
            os.unlink(temp_path)
...
"""
    ...
    def test_FormatDetectionNonexistentFile(self):
        ...
        log_input_expected_error(
            f"filename={filename}", SubtitleParseError, e.exception
        )
    ...
    def test_FormatDetectionWithNonUtf8SrtFile(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", delete=False, encoding="iso-8859-1"
        ) as f:
            f.write(srt_content)
            temp_path = f.name
...
"""
        ...
        try:
            ...
            detected_format = data.metadata.get("detected_format")
            self.assertLoggedEqual(
                "detected .ass format (non-utf)", ".ass", detected_format
            )
            lines_count = len(data.lines)
            ...
            first_line_text = data.lines[0].text if data.lines else ""
            self.assertLoggedEqual(
                "first line text (ass)", "Café à Paris", first_line_text
            )
        finally:
            os.unlink(temp_path)
