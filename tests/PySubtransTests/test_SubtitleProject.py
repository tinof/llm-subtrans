...
class SubtitleProjectTests(SubtitleTestCase):
    ...
    def test_default_initialization(self):
        ...
        self.assertLoggedIsInstance(
            "project.subtitles type", project.subtitles, Subtitles
        )
        ...
        for key in default_settings.keys():
            ...
            self.assertLoggedEqual(
                f"default setting '{key}'", expected_value, actual_value
            )
    ...
    def test_persistent_initialization(self):
        ...
        self.assertLoggedTrue(
            "persistent project.use_project_file", project.use_project_file
        )
    ...
    def test_initialise_project_new_srt(self):
        ...
        self.assertLoggedEqual(
            "project file path", expected_project_file, actual_project_file
        )
    ...
    def test_save_and_reload_project_preserves_settings(self):
        ...
        self.assertLoggedEqual(
            "preserved line count", original_line_count, new_line_count
        )

        original_scene_count = project.subtitles.scenecount
        new_scene_count = new_project.subtitles.scenecount
        self.assertLoggedEqual(
            "preserved scene count", original_scene_count, new_scene_count
        )
    ...
    def test_initialise_project_existing_subtrans(self):
        ...
        settings = SettingsType(
            {"target_language": "German", "movie_name": "Existing Project Movie"}
        )
        ...
        self.assertLoggedEqual(
            "preserved movie_name", "Existing Project Movie", movie_name
        )
    ...
    def test_initialise_project_existing_subtrans_reload(self):
        ...
        settings = SettingsType(
            {"target_language": "Portuguese", "movie_name": "Reload Test Movie"}
        )
        project.UpdateProjectSettings(settings)
...
"""
        ...
        movie_name = new_project.subtitles.settings.get("movie_name")
        self.assertLoggedEqual(
            "preserved target_language", "Portuguese", target_language
        )
        ...
        if new_project.subtitles.originals:
            ...
            self.assertLoggedEqual(
                "reloaded first line", "Modified subtitle line 1", first_line_text
            )
    ...
    def test_properties(self):
        ...
        self.assertLoggedEqual(
            "updated target_language", "Russian", project.target_language
        )
        self.assertLoggedEqual("updated task_type", "translation", project.task_type)
        self.assertLoggedEqual(
            "updated movie_name", "Property Test Movie", project.movie_name
        )
