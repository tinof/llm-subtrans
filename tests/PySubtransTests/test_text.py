...
class TestTextHelpers(LoggedTestCase):
    ...
    linearise_cases = [
        ...
        (
            ["This is a test", "I hope it passes", "I really do"],
            "This is a test | I hope it passes | I really do",
        ),
        ([20, 30, 40], "20 | 30 | 40"),
    ]
        ...
        for input, expected in self.linearise_cases:
            with self.subTest():
                ...
                self.assertLoggedEqual(
                    "linearise", expected, result, input_value=str(input)
                )
    ...
    def test_RemoveWhitespaceAndPunctuation(self):
        for text, expected in self.remove_whitespace_and_punctuation_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "remove whitespace", expected, result, input_value=text
                )
    ...
    def test_NormaliseDialogTags(self):
        for text, expected in self.normalise_tags_cases.items():
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "normalise dialog tags", expected, result, input_value=text
                )
    ...
    def test_BreakDialogOnOneLine(self):
        ...
        for text, expected in self.break_dialog_on_one_line_cases.items():
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "break dialog", expected, result, input_value=text
                )
    ...
    break_long_line_cases = [
        ...
        (
            "A parenthetical (which should be kept together).",
            42,
            10,
            "A parenthetical\n(which should be kept together).",
        ),
        (
            'A line with a "Quote that should not be broken."',
            42,
            10,
    ...
    limit_text_length_cases = [
        ...
        (
            "First sentence. Second sentence can fit too.",
            100,
            "First sentence. Second sentence can fit too.",
        ),
        ...
        (
            "This is exactly thirty-nine characters.",
            39,
            "This is exactly thirty-nine characters.",
        ),
        ...
        (
            "Hello! How are you doing today? I hope well.",
            32,
    ...
    def test_LimitTextLength(self):
        for text, limit, expected in self.limit_text_length_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "limit text length", expected, result, input_value=text
                )
    ...
    def test_ContainsTags(self):
        for text, expected in self.contains_tags_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "contains tags", expected, result, input_value=text
                )

    extract_tag_cases = [
        ("This test has no tags", "tag", ("This test has no tags", None)),
        (
            "This test has a <tag>tagged section</tag>",
            "tag",
            ("This test has a", "tagged section"),
        ),
        (
    ...
    def test_ExtractTag(self):
        for text, tagname, expected in self.extract_tag_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "extract tag", expected, result, input_value=text
                )
    ...
    extract_taglist_cases = [
        ...
        (
            "This test has a <tag>tagged, list, of, words</tag>",
            "tag",
            ("This test has a", ["tagged", "list", "of", "words"]),
        ),
        (
    ...
    def test_ExtractTaglist(self):
        for text, tagname, expected in self.extract_taglist_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "extract tag list", expected, result, input_value=text
                )
    ...
    sanitise_summary_cases = [
        ...
        (
            "Movie Name: Summary of the scene - This is a summary",
            "Movie Name",
            None,
            "This is a summary",
        ),
        ...
        (
            "Scene 1: This is the first scene of Movie Name",
            "Movie Name",
            None,
            "This is the first scene of Movie Name",
        ),
        (
    ...
    def test_SanitiseSummary(self):
        for text, movie_name, max_length, expected in self.sanitise_summary_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "sanitise summary", expected, result, input_value=text
                )
    ...
    remove_filler_cases = [
        ...
        (
            "This, err, sentence is, umm, full of filler words, eh?",
            "This sentence is full of filler words",
        ),
        (
            "This sentence has no filler. Ah, but this one does.",
            "This sentence has no filler. But this one does.",
        ),
    ]
    ...
    def test_RemoveFillerWords(self):
        ...
        for text, expected in self.remove_filler_cases:
            with self.subTest(text=text):
                ...
                self.assertLoggedEqual(
                    "remove filler words", expected, result, input_value=text
                )
    ...
    fullwidth_punctuation_cases = [
        ...
        (
            "안녕하세요,세계? 你好,世界! こんにちは、世界!",
            "안녕하세요，세계? 你好，世界! こんにちは、世界!",
        ),
        ("Hello, world! 你好,世界!", "Hello, world! 你好，世界!"),
        (
            "안녕하세요. Hello, world! 你好,世界!",
            "안녕하세요. Hello, world! 你好，世界!",
        ),
        ("これは、テストです.", "これは、テストです."),
