...
class TestTimeHelpers(LoggedTestCase):
    ...
    get_timedelta_cases = [
        ...
        (
            "01:30:45",
            timedelta(hours=1, minutes=30, seconds=45, milliseconds=0),
        ),  # Without milliseconds
        (
            "30:45",
            timedelta(hours=0, minutes=30, seconds=45, milliseconds=0),
        ),  # Without hours and milliseconds
        (
            "45,250",
            timedelta(hours=0, minutes=0, seconds=45, milliseconds=250),
        ),  # Without hours and minutes
        (
            "01:30:45.123",
            timedelta(hours=1, minutes=30, seconds=45, milliseconds=123),
        ...
        (
            "30:45．123",
            timedelta(hours=0, minutes=30, seconds=45, milliseconds=123),
        ),  # Without hours, full-width period
        (
            "10:20:30,400,500",
            timedelta(hours=10, minutes=20, seconds=30, milliseconds=400),
        ),
        (
            "100:20:30,400,500,600",
            timedelta(hours=100, minutes=20, seconds=30, milliseconds=400),
        ),
        (None, None),
        ("invalid", ValueError),
    ]
    ...
    timedelta_to_text_cases = [
        (
            timedelta(hours=2, minutes=34, seconds=56, microseconds=789000),
            "2:34:56,789",
            True,
        ),
    ...
    def test_TimeDeltaToText(self):
        for value, expected, include_milliseconds in self.timedelta_to_text_cases:
            with self.subTest(value=value):
                result = TimedeltaToText(
                    value, include_milliseconds=include_milliseconds
                )
                self.assertLoggedEqual(
