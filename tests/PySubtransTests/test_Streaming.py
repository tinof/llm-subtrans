...
class MockStreamingTranslationClient(DummyTranslationClient):
    ...
    def __init__(self, settings: SettingsType):
        ...
        self.streaming_responses: dict[str, list[str]] = settings.get_dict(
            "streaming_responses"
        )  # type: ignore
        self.network_error_on_delta: int | None = (
            settings.get_int("network_error_on_delta")
            if settings.get("network_error_on_delta") is not None
            else None
        )
        self.api_error_on_delta: int | None = (
            settings.get_int("api_error_on_delta")
            if settings.get("api_error_on_delta") is not None
            else None
        )
    ...
    def _simulate_streaming(
        self, request: TranslationRequest, deltas: list[str]
    ) -> Translation | None:
...
class MockStreamingProvider(TranslationProvider):
    ...
    def __init__(
        self,
        data: dict[str, SettingType],
        streaming_responses: dict[str, list[str]] | None = None,
    ):
...
class StreamingTests(SubtitleTestCase):
    ...
    def __init__(self, methodName):
        super().__init__(
            methodName, custom_options={"max_batch_size": 10, "stream_responses": True}
        )
    ...
    def test_streaming_event_handling(self):
        ...
        subtitles = (
            SubtitleBuilder(max_batch_size=10)
        ...
        test_data = {
            ...
            "response_map": {"Translate scene 1 batch 1": complete_response},
        }
        ...
        streaming_responses = {"Translate scene 1 batch 1": streaming_chunks}
    ...
    def test_partial_response_processing(self):
        ...
        self.assertLoggedTrue(
            "contains first text", contains_first, input_value=final_response
        )
        self.assertLoggedTrue(
            "contains second text", contains_second, input_value=final_response
        )
    ...
    def test_api_error_handling(self):
        ...
        self.assertLoggedTrue(
            "contains API error", found_api_error, input_value=error_messages
        )
    ...
    def test_concurrent_streaming_requests(self):
        ...
        def create_streaming_translator(thread_id):
            subtitles = (
                ...
                .BuildLine(
                    timedelta(seconds=1),
                    timedelta(seconds=3),
                    f"Thread {thread_id} line 1",
                )
                .BuildLine(
                    timedelta(seconds=4),
                    timedelta(seconds=6),
                    f"Thread {thread_id} line 2",
                )
                .Build()
            )
        ...
        num_threads = 3
        translators_and_subtitles = [
            create_streaming_translator(i) for i in range(num_threads)
        ]
        ...
        for i, (translator, subtitles) in enumerate(translators_and_subtitles):
            thread = threading.Thread(
                target=run_translation, args=(translator, subtitles, i)
            )
        ...
        self.assertLoggedEqual(
            "concurrent translation results", num_threads, len(results)
        )
