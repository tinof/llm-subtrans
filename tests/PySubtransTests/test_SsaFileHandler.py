...
"""
    ...
    def test_parse_string_basic(self):
        ...
        self.assertLoggedEqual(
            "Parsed line count", len(self.expected_lines), len(lines)
        )
    ...
    def test_load_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            "w", delete=False, suffix=".ass", encoding="utf-8"
        ) as f:
    ...
    def test_compose_lines_basic(self):
        ...
        expected_sections = [
            ...
            "[V4+ Styles]",
            "[Events]",
            "Dialogue: 0,0:00:01.50,0:00:03.00,Default,,0,0,0,,Test subtitle",
        ]
        ...
        self.assertIn(
            "Dialogue: 0,0:00:01.50,0:00:03.00,Default,,0,0,0,,Test subtitle", result
        )
...
"""
    ...
    def test_round_trip_conversion(self):
        ...
        self.assertLoggedEqual(
            "Original line count", len(original_lines), len(round_trip_lines)
        )

        # Validate metadata preservation
        self.assertLoggedEqual(
            "Metadata format preserved",
            original_data.metadata["pysubs2_format"],
            round_trip_data.metadata["pysubs2_format"],
        )
        self.assertLoggedIn(
            "Original metadata contains styles", "styles", original_data.metadata
        )
        self.assertLoggedIn(
            "Round-trip metadata contains styles", "styles", round_trip_data.metadata
        )
        self.assertLoggedEqual(
            "Styles preserved",
            original_data.metadata["styles"],
            round_trip_data.metadata["styles"],
        )
...
Dialogue: Marked=0,0:00:01.00,0:00:02.00,Default,,0000,0000,0000,,SSA line"""
    ...
    def test_subtitle_line_to_pysubs2_time_conversion(self):
        ...
        for i, (test_timedelta, expected_ms) in enumerate(test_cases):
            with self.subTest(case=i):
                ...
                self.assertLoggedEqual(
                    f"Timedelta {test_timedelta}", expected_ms, pysubs2_event.start
                )
    ...
    def test_ssa_to_html_formatting_conversion(self):
        ...
        test_cases = [
            ...
            (
                "{\\i1}Mixed {\\b1}formatting{\\b0} here{\\i0}",
                "<i>Mixed <b>formatting</b> here</i>",
            ),
...
"""
        ...
        self.assertLoggedEqual(
            "Italic line", "<i>This is italic text</i>", original_lines[0].text
        )

        self.assertLoggedEqual(
            "Mixed formatting",
            "<b>This is bold</b> and <i>this is italic</i>",
            original_lines[1].text,
        )
        ...
        self.assertLoggedEqual(
            "Round-trip italic text",
            "<i>This is italic text</i>",
            round_trip_lines[0].text,
        )
        self.assertLoggedEqual(
            "Round-trip bold and italic text",
            "<b>This is bold</b> and <i>this is italic</i>",
            round_trip_lines[1].text,
        )
        self.assertEqual(round_trip_lines[2].text, "Normal text with\nline break")

        # Verify original SSA tags are in composed output
        self.assertLoggedIn(
            "Italic SSA tag in output", "{\\i1}This is italic text{\\i0}", composed_ass
        )
        self.assertLoggedIn(
            "Bold SSA tag in output", "{\\b1}This is bold{\\b0}", composed_ass
        )
...
"""
        ...
        self.assertLoggedEqual(
            "Complex tags extracted",
            "{\\pos(100,200)\\an5\\fs20}",
        ...
        self.assertEqual(
            complex_line.metadata["override_tags_start"], "{\\pos(100,200)\\an5\\fs20}"
        )
        ...
        self.assertEqual(
            multi_line.metadata["override_tags_start"], "{\\pos(50,100)}{\\c&H00FF00&}"
        )
        ...
        mixed_line = lines[4]
        self.assertLoggedEqual(
            "HTML conversion with inline preservation",
        ...
        self.assertLoggedTrue(
            "Complex tags restored", "{\\pos(100,200)\\an5\\fs20}" in composed_ass
        )
        self.assertLoggedTrue(
            "Multi-block tags restored", "{\\pos(50,100)}{\\c&H00FF00&}" in composed_ass
        )

        self.assertIn(
            "{\\pos(100,200)\\an5\\fs20}Complex positioned text", composed_ass
        )
        ...
        self.assertLoggedTrue(
            "Round-trip metadata preserved",
            rt_complex.metadata.get("override_tags_start", "")
            == "{\\pos(100,200)\\an5\\fs20}",
        )
    ...
    def test_tag_extraction_functions(self):
        ...
        extraction_cases = [
            ...
            (
                "{\\pos(100,200)\\an5\\fs20}Text",
                {"override_tags_start": "{\\pos(100,200)\\an5\\fs20}"},
            ),
            # Multiple tag blocks
            (
                "{\\pos(50,100)}{\\c&H00FF00&}Text",
                {"override_tags_start": "{\\pos(50,100)}{\\c&H00FF00&}"},
            ),
            # No whole-line tags
            ("Normal text", {}),
            # Inline tags only
        ...
        for ssa_input, expected_metadata in extraction_cases:
            with self.subTest(input=ssa_input):
                ...
                self.assertLoggedEqual(
                    f"Extract: {ssa_input}", expected_metadata, result
                )
        ...
        restoration_cases = [
            ...
            (
                "Text",
                {"override_tags_start": "{\\pos(100,200)}"},
                "{\\pos(100,200)}Text",
            ),
        ...
        for text_input, metadata, expected_result in restoration_cases:
            with self.subTest(text=text_input, metadata=metadata):
                ...
                self.assertLoggedEqual(
                    f"Restore: {text_input} + {metadata}", expected_result, result
                )
        ...
        conversion_cases = [
            ...
            (
                "Text with {\\c&H0000FF&}color{\\c}",
                "Text with {\\c&H0000FF&}color{\\c}",
            ),
            # Mixed case
            (
                "{\\pos(50,100)}Text with {\\c&H00FF00&}inline{\\c} color",
                "Text with {\\c&H00FF00&}inline{\\c} color",
            ),
        ]
    ...
    def test_composite_tags_with_basic_formatting(self):
        ...
        test_cases = [
            ...
            (
                "{\\pos(100,200)\\i1\\b1}Bold italic{\\b0}{\\i0}",
                "<i><b>Bold italic</b></i>",
            ),
            # Complex composite with color and italic
            (
                "{\\pos(100,200)\\c&H00FF00&\\i1}Green italic{\\i0}",
                "<i>Green italic</i>",
            ),
...
"""
        ...
        italic_line = lines[0]
        self.assertLoggedEqual(
            "Composite italic in GUI", "<i>Italic positioned text</i>", italic_line.text
        )
        self.assertEqual(
            italic_line.metadata.get("override_tags_start"), "{\\pos(100,200)}"
        )

        # Verify bold formatting preserved in GUI
        bold_line = lines[1]
        self.assertLoggedEqual(
            "Composite bold in GUI", "<b>Green bold text</b>", bold_line.text
        )
        self.assertEqual(
            bold_line.metadata.get("override_tags_start"), "{\\c&H00FF00&}"
        )

        # Test round-trip - verify both positioning and formatting restored
        composed = self.handler.compose(data)
        self.assertLoggedTrue(
            "Round-trip composite preservation",
            "{\\pos(100,200)}{\\i1}Italic positioned text{\\i0}" in composed,
        )
        self.assertLoggedTrue(
            "Round-trip composite bold preservation",
