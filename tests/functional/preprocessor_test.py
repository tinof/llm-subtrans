...
def preprocess_test(subtitles: Subtitles, logger, options: SettingsType):
    ...
    logger.info(
        "| {:<20} | {:<20} | {:<20} |".format(
            "Original count", "New line count", "Delta"
        )
    )
    logger.info(
        "| {:<20} | {:<20} | {:<20} |".format(
    ...
    options.get_float("min_line_duration") or 0.0  # type: ignore[unused-variable]

    for line in test_lines:
        if (
            max_line_duration > 0.0
            and line.duration.total_seconds() > max_line_duration
        ):
...
def run_tests(directory_path: str, results_path: str | None = None):
    ...
    RunTestOnAllSubtitleFiles(
        preprocess_test, test_options, directory_path, results_path
    )
