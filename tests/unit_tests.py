...
def discover_tests_in_directory(
    loader: unittest.TestLoader,
    test_dir: str,
    base_dir: str,
    handle_import_errors: bool = False,
) -> unittest.TestSuite:
    ...
    if handle_import_errors:
        try:
            return loader.discover(
                test_dir, pattern="test_*.py", top_level_dir=base_dir
            )
...
def discover_tests(base_dir=None, separate_suites=False):
    ...
    try:
        ...
        if gui_available:
            gui_tests = discover_tests_in_directory(
                loader, guisubtrans_dir, base_dir, handle_import_errors=True
            )
        else:
